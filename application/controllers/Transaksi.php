...
class Transaksi extends CI_Controller
{
	...
	public function index()
	{
		...
		if ($this->session->userdata('level') == 'Anggota') {
			$this->data['pinjam'] = $this->db->query(
				...
				FROM tbl_pinjam WHERE status = 'Dipinjam' OR status = 'Menunggu'
	...
	public function pinjam()
	{
		...
		$this->data['nop'] = $this->M_Admin->buat_kode('tbl_pinjam', 'PJ', 'id_pinjam', 'ORDER BY pinjam_id DESC LIMIT 1');
		$this->data['idbo'] = $this->session->userdata('ses_id');
	...
	{
		$this->data['title_web'] = 'Data Konfirmasi Peminjaman Buku';
		$this->data['idbo'] = $this->session->userdata('ses_id');


		$this->data['pinjam'] = $this->db->query("SELECT DISTINCT `pinjam_id`, `anggota_id`, 
				`status`, `tgl_pinjam`, `lama_pinjam`, `tgl_balik`, `tgl_kembali` 
				FROM tbl_pinjam WHERE status = 'Menunggu' ORDER BY pinjam_id DESC");


		$this->load->view('header_view', $this->data);
		$this->load->view('sidebar_view', $this->data);
		$this->load->view('pinjam/konfirmasi_view', $this->data);
		$this->load->view('footer_view', $this->data);
	}
